...
전체 설계는 [DESIGN.md](DESIGN.md) 참조.

**🟢 스토어 게시됨 (2026-07-03, v0.1.0):** [Chrome 웹스토어](https://chromewebstore.google.com/detail/abifielhojkgohomnhabbplnnjjfgmpf) · 항목 ID `abifielhojkgohomnhabbplnnjjfgmpf`
→ Phase 1(MVP) 완료. 다음은 Phase 2(기능 개선) + 유입.
...
## Phase 1 (MVP) — 완료 ✅ (스토어 게시)

- [x] 수집 버튼 삽입 + 현재 페이지 파싱 → service worker 저장(Dexie)
- [x] 주문번호 합산 뷰 + CSV 다운로드(엑셀 한글 BOM) — 대시보드
- [x] 팝업: 누적 건수/최근 수집일 + 주문목록/대시보드 열기
- [x] **멀티페이지 자동 순회** — pageIndex 파라미터로 확정, 라이브 검증(27주문/43상품)
- [x] 스토어 심사용 아이콘/스크린샷/설명 정비 후 제출 → **게시 완료**

## Phase 2 (개선) — 완료 ✅ (v0.2)

- [x] 월별 지출 막대그래프 (Recharts, 취소 제외)
- [x] 규칙 기반 카테고리 자동 분류 — 8개 대분류, 긴 키워드 우선 매칭
      (실제 상품명 31개 검증 전부 정확 / DESIGN 목표 70%)
- [x] XLSX 내보내기 — 주문내역·월별·카테고리별 3개 시트, 열너비·숫자서식·자동필터
      (SheetJS 공식 배포판, 클릭 시에만 로드되는 별도 청크)
- [x] 정확도 개선: 분리배송 중복 집계, 배송비 반영, **취소 건 지출 제외**
- [x] 실사용자 제보 반영: 수집 중지 버튼, 쿠팡 계정별 프로필 분리

**남은 것(Phase 2 DoD 측정):** 설치 300+, 평점 4.5+, 주간 재사용률 30%+
